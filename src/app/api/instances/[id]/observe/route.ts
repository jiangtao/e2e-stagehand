...
export async function POST(
  ...
  try {
    ...
    // 验证请求数据
    const { instruction } = ObserveRequestSchema.parse(body);
    ...
    // 执行页面观察
    const result = await adapter.observe(instruction);

    return NextResponse.json({
      success: true,
      data: {
        instruction: instruction || 'General observation',
        result,
        instanceId,
    ...
    return NextResponse.json({
      success: false,
      error: error instanceof Error ? error.message : 'Failed to observe page'
    }, { status: 500 });
  }
}
